...
def debug_print(sender, position_runtime, odom_runtime, stop_event):
    ...
    while not stop_event.is_set():
        ...
        if robot_pose is None:
            ...
            hold_text = f"hold_dist={distance:6.3f} dx={dx:7.3f} dy={dy:7.3f}"
        ...
        print(
            ...
            f"ch={channels} | "
            f"{pose_text} | {hold_text} | {odom_text}"
        )
...
def main():
    ...
    try:
        print("Starting rotation with position hold test...")
        ...
        if not wait_runtime_ready(position_runtime, odom_runtime):
            print("Rotation move test failed: pose/odometry not ready.")
            return
        ...
        debug_thread = threading.Thread(
            ...
            daemon=True,
            name="rotation_move_debug_thread",
        )
        debug_thread.start()

        print(
            "Rotate with position hold: "
            f"hold=({TARGET_X:.3f}, {TARGET_Y:.3f}), "
            f"target_yaw={TARGET_YAW_DEG:.2f} deg"
        )
        ...
        print("Rotation move test finished.")
        print(result)
