...
TARGET_YAWS = [0.01, 90.0, 180.0, -90.0, 0.01]
HOLD_AFTER_REACHED_SEC = 0.5
...
def debug_print(sender, position_runtime, odom_runtime, stop_event):
    ...
    while not stop_event.is_set():
        ...
        if robot_pose is None:
            ...
            pose_text = f"pos=({float(robot_pose['x']):7.3f},{float(robot_pose['y']):7.3f})"
        ...
        print(
            ...
            f"ch={channels} | {pose_text} | {odom_text}"
        )
...
def main():
    ...
    try:
        print("Starting original rotation loop test...")
        ...
        if not wait_runtime_ready(position_runtime, odom_runtime):
            print("Original rotation test failed: pose/odometry not ready.")
            return
        ...
        debug_thread = threading.Thread(
            ...
            daemon=True,
            name="ori_rot_debug_thread",
        )
        debug_thread.start()

        index = 0
        while True:
            target_yaw = TARGET_YAWS[index % len(TARGET_YAWS)]
            print(f"Rotate to {target_yaw:.2f} deg...")
            ...
            print(result)
            time.sleep(HOLD_AFTER_REACHED_SEC)
            index += 1
