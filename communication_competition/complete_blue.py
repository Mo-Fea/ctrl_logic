...
# Blue-field placeholders mirrored from complete_red.py. Replace with measured values if needed.
...
WAIT_BEFORE_LOCK_SEC = 0.0
WAIT_AFTER_WEAPON_RELEASE_SEC = 5.0
...
def main():
    ...
    try:
        print("Starting complete blue challenge test...")
        ...
        if not wait_runtime_ready(position_runtime, odom_runtime):
            print("Complete blue test failed: runtime data not ready.")
            return
        ...
        if WAIT_BEFORE_LOCK_SEC > 0.0:
            time.sleep(WAIT_BEFORE_LOCK_SEC)

        print("Waiting for visual scanner lock before releasing weapon...")
        with visual_lock:
            print("Visual scanner lock acquired. Releasing weapon and lowering gripper...")
        ...
        print(f"Waiting {WAIT_AFTER_WEAPON_RELEASE_SEC:.1f}s after weapon release...")
        time.sleep(WAIT_AFTER_WEAPON_RELEASE_SEC)
        ...
        if start_move_result is None:
            print("Complete blue test failed: move to -1 failed.")
            return
        ...
        print("Complete blue challenge test completed.")
