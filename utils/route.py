...
def _direction_code(from_pos, to_pos):
    """根据 meilin.py 的位置坐标，计算网格移动方向编码。"""
    if from_pos == to_pos:
        return 0

    from_row, from_col, _ = meilin.pos_to_coord[from_pos]
    to_row, to_col, _ = meilin.pos_to_coord[to_pos]
...
def _height_action(from_pos, to_pos):
    ...
    from_height = meilin.pos_to_coord[from_pos][2]
    to_height = meilin.pos_to_coord[to_pos][2]
...
def path_to_action_matrix(kfs, path):
    """
    将 meilin.plan_path() 的路径转换成 n*5 机器人动作矩阵。

    meilin.py 的路径输入/输出约定：
    - 整数：R2实际移动到的位置
    ...
    # 入口位置如果本身就是R2-KFS，meilin.py会视为自动抓取。
...
def print_action_matrix(action_matrix):
    """按 meilin.py 的路径结果打印 n*5 动作矩阵和简要说明。"""
    print("\n[机器人动作矩阵 n*5]")
